...
# ⚡ Bolt: Precompute dataclass fields to avoid overhead inside request loop
_CARD_FIELDS = frozenset(f.name for f in dataclasses.fields(Card))
...
async def get_cards_by_ids(id_cards: typing.List[str]) -> typing.List[Card]:
    ...
    # ⚡ Bolt: Wrap synchronous meilisearch client search with to_thread to prevent event loop blocking
    def _search():
